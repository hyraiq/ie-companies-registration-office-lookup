...
final class StubHttpClient implements HttpClientInterface
{
    ...
    {
        if ('GET' !== $method) {
            throw new \LogicException('Not implemented: IE Companies Registration Office API client should only use GET requests');
        }

        return match (true) {
            (bool) \preg_match('/^company\/\d+\/c/', $url) => $this->handleCompanyRequest($url, $options),
            default                                        => throw new \LogicException(
                'Not implemented: IE Companies Registration Office API client only responds to "/company/{companyNumber}/c"'
            ),
        };
    }
    ...
    {
        throw new \LogicException('Not implemented: IE Companies Registration Office API client should only be using the request method');
    }
    ...
    private function getMockHttpClient(): MockHttpClient
    {
        ...
        return new MockHttpClient($this->stubResponse, 'https://services.cro.ie/cws/');
    }
}
